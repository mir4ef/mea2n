...
export class Lazy2Service {
  ...
  // it will be used to 'cache' a response and save a trip to the backend (not recommended approach unless you are dealing with data that won't change i.e. image urls - not a perfect example, but the idea is that the image URLs will be pretty static, but the image itself could change (which you don't care about))
